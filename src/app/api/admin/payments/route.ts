...
// Part 31C — Paginated Razorpay order list joined with user info.
// FIXED: amountInr falls back to credits_to_add mapping when amount column is absent.

// Maps known pack sizes (credits_to_add) → INR price as fallback
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ── Apply search (email or order ID) ──────────────────────────────────────
    if (search.trim()) {
